"""Persistent task registry for the Claude control plane."""
...
def claude_registry_path() -> str:
    home = os.environ.get("HERMES_HOME") or os.path.join(os.path.expanduser("~"), ".hermes")
    return os.path.join(home, "claude-control-plane", "tasks.json")
...
class ClaudeTaskRecord:
    ...
    @classmethod
    def from_dict(cls, data: JsonDict) -> "ClaudeTaskRecord":
        return cls(
            ...
            permission_mode=str(data.get("permissionMode") or SAFE_PERMISSION_MODE),
...
class ClaudeTaskRegistry:
    """Small persistent index of Claude work controlled by Hermes."""

    def __init__(self, path: str | None = None) -> None:
        self.path = path or claude_registry_path()
        self._lock = threading.RLock()
        self._loaded = False
        self._records: dict[str, ClaudeTaskRecord] = {}
        self._latest_by_key: dict[str, str] = {}

    def upsert(self, record: ClaudeTaskRecord) -> ClaudeTaskRecord:
        with self._lock:
    ...
    def update(self, task_id: str, **fields: Any) -> ClaudeTaskRecord | None:
        with self._lock:
            with self._file_lock_locked():
                self._load_locked(force=True)
    ...
    def list(self, *, task_key: str | None = None, limit: int = 10) -> list[ClaudeTaskRecord]:
        with self._lock:
            with self._file_lock_locked(exclusive=False):
                self._load_locked(force=True)
    ...
    def _load_locked(self, *, force: bool = False) -> None:
        ...
        if not isinstance(records, list):
            return
        loaded: list[ClaudeTaskRecord] = []
        for raw in records:
            if not isinstance(raw, dict):
                continue
            record = ClaudeTaskRecord.from_dict(raw)
            if record.task_id:
