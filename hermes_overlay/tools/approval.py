...
def request_gateway_approval(approval_data: dict, *, surface: str = "gateway") -> dict:
    """Ask the active gateway session to resolve an approval request.

    This is the public bridge for non-terminal integrations that already have
    their own approval semantics, such as Codex app-server. It reuses the same
    gateway queue, buttons, text fallback, timeout, and /approve /deny handlers
    as terminal command approval.
    """
    session_key = get_current_session_key("")
    if not session_key:
        return {"resolved": False, "choice": None, "reason": "no_session"}
    with _lock:
        notify_cb = _gateway_notify_cbs.get(session_key)
    if notify_cb is None:
        return {"resolved": False, "choice": None, "reason": "no_notify_callback"}

    data = dict(approval_data or {})
    data.setdefault("command", "")
    data.setdefault("description", "Codex requested approval")
    pattern_key = str(data.get("pattern_key") or "codex_app_server_approval")
    data["pattern_key"] = pattern_key
    data.setdefault("pattern_keys", [pattern_key])
    data.setdefault("allow_permanent", False)
    if is_approved(session_key, pattern_key):
        return {"resolved": True, "choice": "session", "preapproved": True}

    decision = _await_gateway_decision(session_key, notify_cb, data, surface=surface)
