...
def test_parse_stream_line_does_not_duplicate_result_after_text_delta() -> None:
    line = json.dumps({
        "type": "result",
        "subtype": "success",
        "session_id": "sess-abc",
    ...
    events = _parse_stream_line(line, "hello")

    assert all(delta == "" for _, delta, _, _, _ in events)
    assert events[-1][0]["stage"] == "turn_completed"
...
def test_claude_session_run_turn_accepts_configured_idle_timeout() -> None:
    signature = inspect.signature(ClaudeCliSession.run_turn)
