...
class FakeClient:
    ...
    def request(self, method: str, params: Optional[dict] = None, timeout: float = 30.0):
        ...
        if method == "thread/resume":
            return {"thread": {"id": (params or {}).get("threadId", "thread-resumed-001")}}
...
class TestLifecycle:
    ...
    def test_resume_thread_uses_thread_resume(self):
        client = FakeClient()
        s = make_session(client, resume_thread_id="thread-existing-001")
        tid = s.ensure_started()

        assert tid == "thread-existing-001"
        assert not [m for (m, _) in client.requests if m == "thread/start"]
        method, params = next(r for r in client.requests if r[0] == "thread/resume")
        assert method == "thread/resume"
        assert params["threadId"] == "thread-existing-001"
        assert params["cwd"] == "/tmp"
    ...
    @pytest.mark.parametrize(
        ...
        [
            {"thread": {"id": "thread-a"}},
            {"thread": {"sessionId": "thread-b"}},
            {"threadId": "thread-c"},
            {"sessionId": "thread-d"},
        ],
    )
    def test_resume_thread_accepts_response_id_variants(self, payload):
        client = FakeClient()
        ...
        expected = (
            ...
            or payload.get("thread", {}).get("sessionId")
...
class TestRunTurn:
    ...
    def test_progress_callback_reports_turn_and_tool_activity(self):
        client = FakeClient()
        client.queue_notification(
            "item/completed",
            item={
        ...
        assert "tool_completed" in stages
...
class TestSessionRetirement:
    ...
    def test_post_tool_watchdog_trusts_native_task_complete(self, tmp_path):
        ...
        native_record = {
            ...
            "payload": {
                ...
                "last_agent_message": "Codex 真实已经完成",
            },
        }
        (
        ...
        assert r.final_text == "Codex 真实已经完成"
        assert r.error is None
        assert r.interrupted is False
        assert r.should_retire is True
        assert any(
            m["role"] == "assistant" and m.get("content") == "Codex 真实已经完成"
            for m in r.projected_messages
        )
        assert not [req for req in client.requests if req[0] == "turn/interrupt"]
        assert any(p["stage"] == "turn_completion_recovered" for p in progress)

    def test_post_tool_watchdog_ignores_native_assistant_messages_without_task_complete(
        self, tmp_path,
    ):
        client = FakeClient()
        client.queue_notification(
        ...
        native_record = {
            ...
            "payload": {
                "type": "message",
                "role": "assistant",
                "content": [{"type": "output_text", "text": "只是一条中间回复"}],
            },
        }
        (
            session_dir / "rollout-2999-01-01T00-00-00-thread-fake-001.jsonl"
    ...
    def test_post_tool_watchdog_resets_on_agent_message_delta(self):
        client = FakeClient()
        client.queue_notification(
            "item/completed",
            item={
