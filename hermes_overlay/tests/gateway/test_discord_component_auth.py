...
def _button_interaction(*, user_id=11111, display_name="0xLL", edit_side_effect=None):
    class _Embed:
        def __init__(self):
            self.color = None
            self.footer = None
    ...
    response = SimpleNamespace(
        ...
        edit_message=AsyncMock(side_effect=edit_side_effect),
    )
...
@pytest.mark.asyncio
async def test_exec_approval_resolves_even_if_discord_edit_fails():
    view = ExecApprovalView(session_key="sess-approval", allowed_user_ids={"11111"})
    interaction = _button_interaction(edit_side_effect=RuntimeError("discord dropped edit"))

    with patch("tools.approval.resolve_gateway_approval", return_value=1) as mock_resolve:
        await view._resolve(interaction, "always", 5, "Approved permanently")

    mock_resolve.assert_called_once_with("sess-approval", "always")
    interaction.response.edit_message.assert_awaited_once()
    assert view.resolved is True


@pytest.mark.asyncio
async def test_slash_confirm_resolves_even_if_discord_edit_fails():
    view = SlashConfirmView(
        session_key="sess-slash",
        confirm_id="confirm-1",
        allowed_user_ids={"11111"},
    )
    interaction = _button_interaction(edit_side_effect=RuntimeError("discord dropped edit"))

    with patch("tools.slash_confirm.resolve", new=AsyncMock(return_value="done")) as mock_resolve:
        await view._resolve(interaction, "once", 2, "Approved once")

    mock_resolve.assert_awaited_once_with("sess-slash", "confirm-1", "once")
    interaction.response.edit_message.assert_awaited_once()
