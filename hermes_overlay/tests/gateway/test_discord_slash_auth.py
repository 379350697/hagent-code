...
def _ensure_discord_mock():
    ...
    if _app is not None and hasattr(_app, "Group"):
        group_cls = _app.Group
        if not hasattr(group_cls, "command"):
            def _group_command(self, *, name, description):
                def decorator(fn):
            ...
            try:
                group_cls.command = _group_command
            except Exception:
                pass
...
# ---------------------------------------------------------------------------
# Platform-decoupled unauthorized logging
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_unauthorized_attempt_does_not_notify_telegram(adapter):
    ...
    telegram_adapter.send.assert_not_awaited()
...
@pytest.mark.asyncio
async def test_notify_does_not_fall_back_to_slack(adapter):
    from gateway.session import Platform
...
# ---------------------------------------------------------------------------
# Unauthorized logging remains platform-local
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_notify_ignores_telegram_soft_fail_without_send(adapter):
    """Discord unauthorized logging must not call Telegram or Slack."""
    from gateway.session import Platform
...
@pytest.mark.asyncio
async def test_notify_ignores_telegram_truthy_success_without_send(adapter):
    """Even configured home channels stay untouched by Discord auth logging."""
    from gateway.session import Platform
