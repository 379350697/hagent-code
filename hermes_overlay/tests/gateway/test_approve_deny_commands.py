...
class TestBlockingGatewayApproval:
    ...
    def test_request_gateway_approval_caches_session_choice(self):
        """Allow Session should suppress repeated Codex bridge prompts."""
        from tools.approval import (
        ...
        session_key = "codex-session-cache"
        notified = []
        ...
        try:
            first = request_gateway_approval(
                {
                    "command": "rg x",
            ...
            second = request_gateway_approval(
                {
                    "command": "rg y",
                    "description": "Codex exec",
                    "pattern_key": "codex_app_server_approval",
        ...
        assert first == {"resolved": True, "choice": "session"}
        assert second["resolved"] is True
        assert second["preapproved"] is True
        assert len(notified) == 1

    def test_request_gateway_approval_always_respects_allow_permanent_gate(self):
        """Always from a non-permanent Codex prompt is session-scoped only."""
        from tools.approval import (
            is_approved,
            register_gateway_notify,
            request_gateway_approval,
        ...
        session_key = "codex-no-permanent"
        pattern_key = "codex_app_server_approval"
        ...
        assert result == {"resolved": True, "choice": "always"}
        assert is_approved(session_key, pattern_key) is True
        assert pattern_key not in _permanent_approved
