...
def _ensure_discord_mock():
    ...
    if _app is not None and not hasattr(_app, "autocomplete"):
        try:
            _app.autocomplete = _fake_autocomplete
        except Exception:
            pass
...
def test_codex_workspace_uses_native_subcommands(adapter):
    adapter._register_slash_commands()

    codex = adapter._client.tree.commands["codex"]
    codex_children = getattr(codex, "_children", {})
    workspace = codex_children["workspace"]
    workspace_children = getattr(workspace, "_children", {})

    assert {"list", "current", "set", "clear"} <= set(workspace_children)
    assert "action" not in workspace_children
    assert "value" not in workspace_children

    signature = inspect.signature(workspace_children["set"].callback)
...
async def test_simple_slash_defer_failure_does_not_dispatch(adapter):
    ...
    await adapter._run_simple_slash(interaction, "/codex continue test", echo=True)
...
async def test_codex_registers_diagnostics_subcommands(adapter):
    ...
    assert {"events", "doctor", "repair"} <= set(codex_children)

    await codex_children["doctor"].callback(interaction)
    adapter._run_simple_slash.assert_awaited_once_with(
        interaction,
        "/codex doctor",
        echo=True,
