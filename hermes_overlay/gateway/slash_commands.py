...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_codex_command(self, event: MessageEvent) -> str:
        """Handle /codex task-control commands via the platform-neutral service."""
        ...
        if source is not None:
            try:
                ...
                logger.debug("Could not resolve /codex admin policy", exc_info=True)
        loop = asyncio.get_running_loop()

        def _codex_approval_notify(approval_data: dict) -> None:
            if adapter is None or not approval_chat_id:
                raise RuntimeError("Codex 审批通道不可用：当前平台没有可用适配器")

            async def _send_approval() -> None:
                command = str(approval_data.get("command") or "")
                description = str(
                    approval_data.get("description") or "Codex 请求审批"
                )
                ...
                if getattr(type(adapter), "send_exec_approval", None) is not None:
                    ...
                    if getattr(result, "success", False):
                        return
                    logger.warning(
                        "Codex approval button send failed, falling back to text: %s",
                ...
                if not getattr(send_result, "success", False):
                    raise RuntimeError(
                        f"Codex 审批通道不可用：{getattr(send_result, 'error', '')}"
                    )
        ...
        def _codex_progress_notify(progress_data: dict) -> None:
            if adapter is None or not approval_chat_id:
                return
            ...
            async def _send_progress() -> None:
                ...
                if progress_message_id:
                    try:
                        ...
                        logger.debug("Codex progress edit failed", exc_info=True)
                        send_result = None
                if not getattr(send_result, "success", False):
                    send_result = await adapter.send(
                        approval_chat_id,
                ...
                if not getattr(send_result, "success", False):
                    logger.debug(
                        "Codex progress delivery failed: %s",
        ...
        request = CommandRequest(
            ...
            approval_notify=_codex_approval_notify if adapter is not None else None,
            progress_notify=_codex_progress_notify if adapter is not None else None,
            is_admin=is_admin,
        )
