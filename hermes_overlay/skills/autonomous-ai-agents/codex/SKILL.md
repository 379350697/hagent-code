...
Codex session. Treat these as user intent for Hermes/Codex delegation; do not
require the user to know hidden phrases.

| Command | Action |
|---------|--------|
| `codex continue <task>` | Resume the current Codex session for this project/thread |
| `codex new <task>` | Start a fresh Codex session |
| `codex status` | Show the current Codex workspace, model/reasoning setting, session mode, and session id if known |
| `codex sessions` | List recent/known Codex sessions when Hermes has that context available |

Default behavior:

1. Same project and same conversation thread: continue the current Codex session.
2. New unrelated task: start a new Codex session.
...
| Alias | Treat as |
|-------|----------|
| `codex 接着 <task>` | `codex continue <task>` |
| `codex 新开 <task>` | `codex new <task>` |

When using the Hermes `codex_app_server` tool, pass `threadId` to continue and
omit `threadId` to start a new session. Always surface the selected workspace,
model, reasoning effort, session mode, and session id from the tool's
`Codex task started` block when reporting progress to the user.
