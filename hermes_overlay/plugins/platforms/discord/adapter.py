...
def _classify_discord_interaction_error(exc: BaseException) -> str:
    status = getattr(exc, "status", None)
    code = getattr(exc, "code", None)
    text = str(exc).lower()
    if status == 429 or "rate limited" in text or "429" in text:
        return "rate_limited"
    if code == 10062 or "unknown interaction" in text:
        return "unknown_interaction"
    if status == 404 or "not found" in text:
        return "not_found"
    if "already been acknowledged" in text or code == 40060:
...
def _read_command_sync_guild_ids() -> list[int]:
    """Return guild IDs that should receive immediate slash-command sync."""
    raw_values: list[Any] = [
        os.getenv("DISCORD_COMMAND_SYNC_GUILDS", ""),
    ...
    try:
        ...
        cfg = read_raw_config() or {}
        discord_cfg = cfg.get("discord", {}) or {}
        raw_values.append(discord_cfg.get("command_sync_guilds"))
        raw_values.append(discord_cfg.get("guild_id"))
    except Exception:
        pass
    ...
    for raw in raw_values:
        ...
        for value in values:
            try:
                ...
                continue
            if guild_id > 0 and guild_id not in seen:
                seen.add(guild_id)
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def _run_post_connect_initialization(self) -> None:
        ...
        try:
            ...
            guild_ids = _read_command_sync_guild_ids()
            if guild_ids:
                for guild_id in guild_ids:
                    guild = discord.Object(id=guild_id)
                    self._client.tree.copy_global_to(guild=guild)
                    synced = await asyncio.wait_for(
                        self._client.tree.sync(guild=guild),
                        timeout=30,
                    )
                    logger.info(
    ...
    async def _reject_slash(
        ...
        # Keep platform adapters independent: do not notify Telegram/Slack from
        # inside Discord. A separate observer/hook can consume this log if the
        # operator wants cross-platform alerting.
        await self._notify_unauthorized_slash(
            user_name, user_id, chan_id, guild_id, command_text, reason,
        )
    ...
    async def _notify_unauthorized_slash(
        ...
        """Record unauthorized slash attempts without touching other platforms."""
        logger.info(
            "[Discord] Unauthorized slash recorded: user=%s id=%s channel=%s "
            "guild=%s cmd=%r reason=%r",
            user_name,
            user_id,
            chan_id,
    ...
    async def _run_simple_slash(
        ...
        *,
        ...
        # Defer FIRST — Discord requires a response within 3 seconds.  If the
        # platform acknowledgement fails, do not dispatch into Hermes/Codex:
        # the user would see "application did not respond" while work runs in
        # the background.
        try:
            await interaction.response.defer(ephemeral=True)
        except Exception as exc:
            kind = _classify_discord_interaction_error(exc)
            logger.warning(
                "Discord interaction failed (%s): could not defer slash %r: %s",
                kind,
                command_text,
                exc,
                exc_info=True,
        ...
        # Authorization happens after the fast ack. _reject_slash edits the
        # deferred ephemeral response when needed.
        ...
        try:
            ...
            kind = _classify_discord_interaction_error(exc)
            logger.debug("Discord interaction cleanup failed (%s): %s", kind, exc)
            if followup_msg:
                try:
                    await interaction.followup.send(followup_msg, ephemeral=True)
                except Exception as followup_exc:
                    logger.warning(
                        "Discord interaction fallback delivery failed (%s): %s",
                        _classify_discord_interaction_error(followup_exc),
                        followup_exc,
                    )
    ...
    def _register_slash_commands(self) -> None:
        ...
        # ── /codex command group with subcommands ──────────────────────
        # Using Discord native subcommands so each parameter is its own
        # field — no more space-truncation of the task description.

        # Remove any stale single-param /codex left over from an older
        # registration, then replace it with the native subcommand group.
        try:
            tree.remove_command("codex")
        except Exception:
            pass

        codex_group = discord.app_commands.Group(
            name="codex",
            description="Codex 任务控制",
        )

        @codex_group.command(name="new", description="新开 Codex 任务")
        ...
        async def _codex_continue_task_autocomplete(
            ...
            try:
                ...
                if record is None:
                    label = "当前：没有选中的 Codex 会话，建议先用 /codex new"
                else:
        ...
        async def _codex_status(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/codex status", echo=True)

        @codex_group.command(name="sessions", description="列出近期 Codex 会话")
        async def _codex_sessions(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/codex sessions", echo=True)

        @codex_group.command(name="events", description="查看当前 Codex 会话事件")
        async def _codex_events(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/codex events", echo=True)

        @codex_group.command(name="doctor", description="诊断 Codex app-server 链路")
        async def _codex_doctor(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/codex doctor", echo=True)

        @codex_group.command(name="repair", description="预览 Codex 历史误判修复")
        async def _codex_repair(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/codex repair recovered", echo=True)

        async def _codex_session_autocomplete(
            interaction: "discord.Interaction", current: str,
        ) -> list:
            try:
                allowed, _reason = self._evaluate_slash_authorization(interaction)
            ...
            try:
                ...
                logger.debug("[%s] Failed to build Codex session autocomplete", self.name, exc_info=True)
                return []
        ...
        @codex_group.command(name="select", description="选择历史 Codex 会话")
        @discord.app_commands.describe(session="历史会话")
        @discord.app_commands.autocomplete(session=_codex_session_autocomplete)
        async def _codex_select(interaction: discord.Interaction, session: str):
            await self._run_simple_slash(interaction, f"/codex select {session}", echo=True)

        @codex_group.command(name="resume", description="接续指定历史 Codex 会话")
        @discord.app_commands.describe(session="历史会话", task="任务描述")
        @discord.app_commands.autocomplete(session=_codex_session_autocomplete)
        async def _codex_resume(interaction: discord.Interaction, session: str, task: str):
            await self._run_simple_slash(interaction, f"/codex resume {session} {task}", echo=True)

        workspace_group = discord.app_commands.Group(
            name="workspace",
            description="选择或查看 Codex 工作区",
        )
        ...
        async def _codex_workspace_autocomplete(
            ...
            if not allowed:
                return []
            try:
                from gateway.control_planes.codex.workspaces import (
                ...
                logger.debug("[%s] Failed to build Codex workspace autocomplete", self.name, exc_info=True)
                return []
        ...
        @workspace_group.command(name="list", description="列出本机 Git 工作区")
        async def _codex_workspace_list(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/codex workspace list", echo=True)

        @workspace_group.command(name="current", description="查看当前 Codex 工作区")
        async def _codex_workspace_current(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/codex workspace current", echo=True)

        @workspace_group.command(name="set", description="选择 Codex 工作区")
        @discord.app_commands.describe(repo="Git 仓库项目")
        @discord.app_commands.autocomplete(repo=_codex_workspace_autocomplete)
        async def _codex_workspace_set(interaction: discord.Interaction, repo: str):
            await self._run_simple_slash(interaction, f"/codex workspace set {repo}", echo=True)

        @workspace_group.command(name="clear", description="清除当前 Codex 工作区选择")
        async def _codex_workspace_clear(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/codex workspace clear", echo=True)

        codex_group.add_command(workspace_group)

        @codex_group.command(name="diff", description="查看上次 Codex 任务的 git 改动")
        async def _codex_diff(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/codex diff", echo=True)

        @codex_group.command(name="stop", description="中断正在执行的 Codex 任务")
        async def _codex_stop(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/codex stop", echo=True)

        @codex_group.command(name="steer", description="向执行中的 Codex 注入指令")
        @discord.app_commands.describe(text="要注入的指令")
        async def _codex_steer(interaction: discord.Interaction, text: str):
            await self._run_simple_slash(interaction, f"/codex steer {text}", echo=True)

        @codex_group.command(name="plan", description="以计划模式启动 Codex 任务")
        @discord.app_commands.describe(task="任务描述")
        async def _codex_plan(interaction: discord.Interaction, task: str):
            await self._run_simple_slash(interaction, f"/codex plan {task}", echo=True)

        @codex_group.command(name="permissions", description="设置 Codex 沙盒权限")
        @discord.app_commands.describe(mode="选择权限模式")
        @discord.app_commands.choices(mode=[
            discord.app_commands.Choice(name="Default（沙盒 + 按需审批）", value="default"),
            discord.app_commands.Choice(name="Approve for me（沙盒 + 自动批准）", value="approve-for-me"),
            discord.app_commands.Choice(name="Read Only（只读 + 按需审批）", value="read-only"),
            discord.app_commands.Choice(name="Full Access（全权限 + 免审批）", value="full-access"),
        ])
        async def _codex_permissions(interaction: discord.Interaction, mode: str):
            await self._run_simple_slash(interaction, f"/codex permissions {mode}", echo=True)
...
def _define_discord_view_classes() -> None:
    ...
    class ExecApprovalView(discord.ui.View):
        ...
        async def _resolve(
            ...
            # Unblock the waiting agent thread before touching Discord UI.
            # A transient Discord edit failure must not strand Codex until
            # the approval prompt times out.
            try:
            ...
            try:
                ...
                logger.warning(
                    "Discord approval message update failed after resolving session %s "
                    "(choice=%s, user=%s): %s",
                    self.session_key, choice, interaction.user.display_name, exc,
                )
    ...
    class SlashConfirmView(discord.ui.View):
        ...
        async def _resolve(
            ...
            result_text = ""
            # Resolve before editing the Discord message. If Discord drops
            # the interaction response, the gateway command should still
            # proceed instead of leaving the prompt pending until timeout.
            try:
            ...
            try:
                ...
                logger.warning(
                    "Discord slash-confirm message update failed after resolving session %s "
                    "(choice=%s, user=%s): %s",
                    self.session_key, choice, interaction.user.display_name, exc,
                )

            # If the handler returns a follow-up message, post it in the same channel.
            if result_text:
                try:
                    await interaction.followup.send(result_text)
                except Exception as exc:
                    logger.warning(
                        "Discord slash-confirm follow-up failed for session %s "
                        "(choice=%s, user=%s): %s",
                        self.session_key, choice, interaction.user.display_name, exc,
                    )
